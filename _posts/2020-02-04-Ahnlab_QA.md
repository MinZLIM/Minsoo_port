...
### Object 수행 업무
#### Test 환경 구성
 트래픽이 구동하는 OS 및 tool 의 설정에 따라 트래픽 세부 설정들이 변경됨. 이에 따라 Test 환경을 필요로 하게 되었으며, VM 을 통하여 각 서비스 서버들을 구성하며 실 트래픽 테스트를 진행 함.
<br />
Detail info : [Maintain Test Server info](https://minzlim.github.io/Minsoo_port/2020/02/02/Maintain_Server)

#### 자동화 Test 수행
Python3를 통한 객체 랜덤값 및 최대값 추가 Test 수행
...
> TCP 세션 생성/종료간 flag 상태 확인 기능을 휘한 bash shell 스크립트 기반 tool 생성
...
## Content Filter
### Content Filter info
Content Filter 기능은 Proxy 및 Non Proxy 환경에서 L7 프로토콜의 header 및 Body 정보를 기반으로 분석하여 악성 코드 및 행위를 탐지하기 위한 기능으로 다음 기능 하부 기능들을 제공한다. 

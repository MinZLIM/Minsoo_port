...
7. [Network Performance Test](#network-performance-test)
    * [Firewall](#firewall)
...
## Trouble Shooting
QA 활동 진행 간 검증 Coverage 에 포함되지 않는 환경에서 site 이슈가 발생할 수 있으며,  TrusGuard 는 Netowrk 환경 UTM 장비이기 때문에 검증을 하지 않은 환경에서 이슈가 나올 확률이 높으며 , site 에서 

---

## Network Performance Test
Appliance 제품인 TrusGuad 의 Network 성능 측정을 위해 IXIA,BreakPoing 를 활용하여 성능측정(Throughput,CPS,CC,Latency)을 수행 <br />
...
타 보안 기능과 연관되지 않아 최대 성능이 측정되며 , fastpath 지원. 
예시 측정 구성
 >  각 인터페이스가 1Gbps를 제공하고 4 Pair 로 양방향 디렉션인 환경에서 총 8Gbps 가 최고 Throughput 성능 
...
예시 측정 구성
 > EX) Firewall + AV / Firewall + AS / etc
